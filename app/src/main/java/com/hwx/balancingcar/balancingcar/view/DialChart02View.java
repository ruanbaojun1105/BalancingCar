	...
	private void initView()
	 ...
	 @Override
		...
		{		
			...
			chart.getPlotAxis().get(0).getAxisPaint().setColor(Color.rgb(192, 192, 192));
			chart.getPlotAxis().get(0).getAxisPaint().setStrokeWidth(3);
			chart.getPlotAxis().get(1).getFillAxisPaint().setColor(Color.rgb(59, 140, 163) );//47, 199, 140
		
			chart.addCircleAxis(0.2f, Color.rgb(28, 111, 84));
			chart.addCircleAxis(0.15f, Color.rgb(62, 175, 135));
		}
		...
		private void addAttrInfo()
		{
			...
			paintTB.setColor(Color.rgb(79, 79, 79));
			paintTB.setTextAlign(Align.CENTER);
			paintTB.setTextSize(25);
			plotAttrInfo.addAttributeInfo( XEnum.Location.TOP, "MAX: "+max+"km/h", 0.9f, paintTB);
			
			Paint paintBT = new Paint();
			paintBT.setColor(Color.rgb(79, 79, 79));
			paintBT.setTextAlign(Align.CENTER);
			paintBT.setTextSize(30);

			plotAttrInfo.addAttributeInfo(XEnum.Location.BOTTOM,
					"平均速率: "+ Float.toString( /*MathHelper.getInstance().round(mPercentage * max,2)*/(mPercentage*(float) max))+"km/h", 0.8f, paintBT);
		}
